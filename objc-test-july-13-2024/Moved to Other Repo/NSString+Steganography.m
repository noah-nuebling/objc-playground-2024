...
- (NSArray *)secretMessageStartSequence {
    NSArray *result = @[@0x200B, @0x200C, @0x200B, @0x200D, @0x200B];
    return result;
}
...
- (BiMap<NSNumber *, NSNumber *> *)quaternaryDigitToZeroWidthCharacterMap {
        
    static BiMap *_characterMap = nil;
    if (_characterMap == nil) {
        _characterMap = [[BiMap alloc] initWithDictionary:@{
            @-1:    @0x200B,
            @0:     @0x200C,
            @1:     @0x200D,
            @2:     @0x2060,
...
- (NSArray<NSString *> *)secretMessages {
    ...
    NSString *pattern = @"\u200B\u200C\u200B\u200D\u200B"       /// Start sequence
                        "(?:(?:[\u200C\u200D\u2060\u2062]{4})*)"  /// Arbitrary sequence of the 4 characters encoding, 0,1,2,3. Sequence needs to be divisible by 4 since 4 quaternary digits contain one UTF-8 char.
                        "\u200B\u200D\u200B\u200C\u200B";       /// End sequence
...
- (NSString *)decodedAsSecretMessage {
    ...
    for (NSNumber *character in [coreMessage UTF32Characters]) {
        NSNumber *digit = [[self quaternaryDigitToZeroWidthCharacterMap] keyForValue:character];
...
- (NSString *)encodedAsSecretMessage {
    ...
    for (NSNumber *digit in digits) {
        NSNumber *zeroWidthCharacter = [[self quaternaryDigitToZeroWidthCharacterMap] valueForKey:digit];
...
+ (NSString *)stringWithQuaternaryArray:(NSArray<NSNumber *> *)digits {
    ...
    for (int i = 0; i < digits.count; i += 4) {
        ...
        for (int j = 0; j < 4; j++) {
            ...
            [byte addObject:@((quartDigit & 1) != 0)]; /// Measuer small bit of quart digit
        }
...
///
/// Space-inefficient encodings
///

/// Discussion:
///     These encodings use just 2+1 zero-width characters, to create a base-2 encoding. There's a character limit of 512 in the UIStrings we can retrieve inside an XCUITest, which lead to problems,
